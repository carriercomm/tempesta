...
kclient_connect(int descidx)
{
	...
	ret = sock_create_kern(kclient_server_address.sa.sa_family,
	...
	ss_set_proto(sock, &desc->proto, descidx, &kclient_hooks);
	ss_set_callbacks(sock->sk);
	ret = ss_connect(sock, &kclient_server_address.sa,
...
kclient_connect_complete(struct sock *sk)
{
	...
	BUG_ON(desc->sock && (desc->sock != sk->sk_socket));
...
kclient_release_sockets(void)
{
	...
	for (i = 0; i < KCLIENT_NTHREADS; i++) {
		for (k = 0; k < KCLIENT_NCONNECTS; k++) {
			if (kclient_desc[i][k].sock) {
				sock_release(kclient_desc[i][k].sock);
				kclient_desc[i][k].sock = NULL;
			}
		}
	}
}
